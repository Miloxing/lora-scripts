...
# Train Model | 训练模式
model="sdxl_lora" #lora、db、sdxl_lora、sdxl_db、contralnet(unfinished)
...
training_comment="maouii"	# training_comment | 训练介绍，可以写作者名或者使用触发关键词
...
max_train_epoches=36  # max train epoches | 最大训练 epoch
save_every_n_epochs=6 # sa2ve every n epochs | 每 N 个 epoch 保存一次
...
# wandb 
wandb_api_key="15441b8e4eeaf36dcf694dc0c69205b0f5416a10"
log_tracker_name=$output_name

# Sample output | 出图
enable_sample=0 #开启出图
sample_every_n_epochs=2 #每n个epoch出一次图
sample_prompts="./train/tac_anime/1.txt"
...
algo="loha" # algo参数，制定训练lycoris模型种类，包括lora(locon)、loha、IA3以及lokr、dylora 。5个可选
